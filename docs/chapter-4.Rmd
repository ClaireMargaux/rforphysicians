...
ggplot(df, aes(x = x, y = y)) +
  geom_area(data = df_mid,   fill = "skyblue", alpha = 0.6) +   # middle area
  geom_area(data = df_left,  fill = "tomato",  alpha = 0.8) +   # left tail (on top)
  geom_area(data = df_right, fill = "tomato",  alpha = 0.8) +   # right tail (on top)
  geom_line(size = 1) +
...
* The data used to carry out the test should either be *sampled independently* from the two populations being compared or be fully paired. This is in general not testable from the data, but if the data are known to be dependent (e.g. paired by test design), a dependent test has to be applied.
...
* `paired`: a logical indicating whether you want a paired t-test. As a reminder, "paired" means there is an obvious and meaningful one-to-one correspondence between the data in the first set and the data in the second set, e.g. blood pressure in a set of patients before and after administering a medicine.
* `conf.level`: confidence level of the interval (default: 5%).
