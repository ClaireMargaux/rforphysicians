...
head(demo[demo[, "RIDAGEYR"] > 50, ])  # Rows where age > 50
head(demo[demo[, "DMDHHSIZ"] < 3, ])  # Rows where total number of people in the household greater than 3
head(demo[demo[, "DMDHHSIZ"] >= 3, ])  # Rows where total number of people in the household greater or equal 3

# Combine logical vectors using "&" (AND), "|" (OR), and "!" (NOT)
head(demo[(demo[, "RIDAGEYR"] > 50 & demo[, "RIAGENDR"] == "Female"), ])  # Both conditions must be true
head(demo[(demo[, "DMDHHSIZ"] < 3 | demo[, "RIAGENDR"] == "Male"), ])  # One condition must be true
```
