...
✏️ Exercise on comparing means: Compare the systolic blood pressure (1st reading, in mmHg, `BPXSY1`) between males and females (`RIAGENDR`). To choose the right test, determine if the data is roughly normal or skewed.

✏️ Exercise on comparing means n°2: Compare the age of the household reference person (`DMDHRAGE`) between participants who smoke at least 100 cigarettes in life (`SMQ020 == "Yes"`) and non-smokers (`SMQ020 == "No"`). To choose the right test, determine if the data is roughly normal or skewed.

✏️ Exercise on comparing proportions: Test whether smoking at least 100 cigarettes in life (`SMQ020 == "Yes"` and `SMQ020 == "No"`) differs by marital status of the household reference person (`DMDHRMAR == "Married"` vs every other marital status).
 
✏️ Exercise on comparing proportions n°2: Test the association between cigar smoking (`SMQ680`) and pipe smoking (`SMQ690`).

✏️ Exercise on comparing more than two groups: Compare mean BMI (`BMXBMI`) across age groups.

💡 Hint: remember how we used the function `cut()` to create age groups in Chapter 3.

✏️ Exercise on comparing more than two groups: Compare waist circumference (`BMXWAIST`) across different smoking frequency categories (`SMQ040`).

✏️ Exercise on normality: Check visually if BMI (BMXBMI) is normally distributed.

✏️ Exercise on correlation: Compute the Pearson correlation between weight (BMXWT) and BMI (BMXBMI). Compute the Spearman correlation between height (BMXHT) and waist circumference (BMXWAIST).

✏️ Exercise on survival analysis: Using the NHANES mortality data (MORTSTAT, PERMTH_INT), plot the Kaplan–Meier survival curve by gender. Then perform a Log-Rank test to compare survival curves.

✏️ Exercise on linear regression: Fit a linear regression model predicting systolic blood pressure (BPXSY1) using age (RIDAGEYR) and BMI (BMXBMI) as predictors. Interpret the coefficients.

✏️ Exercise on logistic regression: Predict the probability of being a smoker (SMQ020 == "Yes") using age and BMI as predictors. Interpret the coefficients.

💡 Hints

### Solutions

☑️ Exercise on comparing means
...
☑️ Exercise on comparing means n°2
...
☑️ Exercise on comparing proportions: Chi-square test
...
☑️ Exercise on comparing proportions n°2: Fisher’s exact test

(`SMQ680`) and pipe smoking (`SMQ690`).
...
☑️ Exercise on comparing more than two groups – ANOVA

☑️ Exercise on comparing more than two groups – Kruskal-Wallis test

☑️ Exercise on normality: qqplot

☑️ Exercise on correlation: pearson and spearman

☑️ Exercise on survival analysis: km + log rank

☑️ Exercise on linear regression:

☑️ Exercise on logistic regression:
