...
`kruskal.test(formula, data, ...)`: Performs a Kruskal-Wallis rank sum test.
...
Tests for deviations from normality, determining when to use parametric vs. non-parametric tests.

But there is an important remark to make here: With large enough sample sizes ($> 30$ or $40$), the violation of the normality assumption should not cause major problems. This implies that we can use parametric procedures even when the data are not normally distributed. The central limit theorem tells us that in large samples, the sampling distribution ($\neq$ data distribution!) tends to be normal, regardless of the shape of the data; and means of random samples from any distribution will themselves have normal distribution.

We can look for normality visually by using normal Q-Q-plots (Q-Q-plots comparing the data distribution to a normal distribution) or by significance tests (hypothesis tests comparing the sample distribution to a normal one). It is important to ascertain whether data show a **serious deviation** from normality.

## Visual assessments

Visual inspection of the distribution may be used for assessing normality, although this approach does not guarantee that the distribution is normal.

Many methods can be used to assess normality visually:
...
We already discussed histograms and box plots in Chapter 3, and those are not the most adequate tools to assess a given distribution.

A P-P plot compares the empirical cumulative distribution function of a data set with a specified, theoretical cumulative distribution function (e.g. the CDF of the normal distribution). A Q-Q plot compares the quantiles of a data distribution with the quantiles of a standardized, theoretical distribution from a specified family of distributions (e.g. the normal distribution). 

## Q-Q-plots

This approach is not a statistical test, but a very valuable visualization tool to assess the normality of a data sample, and can already allow to judge if the data show a serious deviation from normality. 

A Q–Q plot (quantile–quantile plot) is a probability plot, a graphical method for comparing two probability distributions by plotting their quantiles against each other. If the two distributions being compared are similar, the points in the Q–Q plot will approximately lie on the **identity line $y = x$**.

Q–Q plots can be used to compare collections of data, or theoretical distributions (and in particular: the normal distribution). 
