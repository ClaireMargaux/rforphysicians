...
demo <- read.csv("C:/GitHub/rforphysicians/data_sets/DEMO_G.csv") # Demographics (cycle G = 2011–2012)
...
✏️ Exercise 1: import the `demo`, `bpx`, `bmx` and `smq` data sets from the [`data_sets` folder](https://github.com/ClaireMargaux/rforphysicians/tree/main/data_sets) into R.
...
✏️ Exercise 2: inspect the structure of the `demo` data set, look at different entries, get familiar with those commands.

### Exercise 3

✏️ Exercise 3: generate a new data frame selecting only the female patients that are above 18 years old and that took the interview in Spanish.
...
directory <- "C:/GitHub/rforphysicians/data_sets/" # Use your own working directory here
...
☑️ Exercise 2: Did you manage to select a specific column you were interested in? Were you able to check how many men and women are included in the data set? Here are a few examples of operations you can use to explore the data set.
