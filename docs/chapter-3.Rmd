...
✏️ Exercise 1: Plot the first, second and third systolic blood pressure readings by gender using base R. Display all three plots (1st, 2nd and 3rd readings) in the same output panel.

💡 *Hint:* Try using the `par()` function (e.g. `par(mfrow=c(2,3))`; do not forget to reset it to `par(mfrow=c(1,1))` afterwards!)

## Exercise 2

✏️ Exercise 2: Create a histogram showing the systolic blood pressure (1st reading) for each smoking status category (excluding those who refused to provide their smoking status) using base R. Set the number of breaks to 20. You should get three different plots. Display them all in the same output panel.
...
✏️ Exercise 3: Using the same age groups as in the examples, create a bar plot showing the median BMI by age group using `ggplot2`. Then produce two further bar plots showing the same data for each gender, and display them in the same output panel, using the same y-axis to allow for better comparison.
...
💡 *Hints:* Try using the `slice_tail()` function.
