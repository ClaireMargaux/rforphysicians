...
💡 Be careful before every analysis to make sure that the variables you're working on have the data type that you need! Let me illustrate this with a short example.

```{r}
# Let us assume that you have a data set with 3 groups, e.g., control, treatment 1, and treatment 2, and you coded them initially as 1, 2, and 3
df <- data.frame(
  Group = c(1, 2, 3, 2, 3),
...
# Check the data type of the "Group" column
str(df$Group)
# We see that the variable "Group" is coded as numeric, which is not ideal!
# If we used this in a regression, the model would treat it as numeric and assume a ranking (control < treatment 1 < treatment 2), which is not what we want.

# That's why we recode "Group" as a factor
